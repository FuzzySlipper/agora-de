...
| `den-k8-overlay-labels-visible` | Agent overlay capture taken after native app focus changes; must prove overlay annotations and native app pixels in the same output image. |
...
each target, captures after focus changes, and closes launched surfaces. The
capture packet includes `overlayPixelClassification` and fails unless the same
physical output image contains overlay annotation pixels and native app pixels.
