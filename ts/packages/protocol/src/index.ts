export type {
  AppearanceApplyRequest,
  AppearanceApplyResponse,
  AppearanceSettings,
  AppearanceSettingsState,
  AppearanceThemeSummary,
  AppearanceValidateRequest,
  AdminEscalationSummary,
  AgentInfo,
  AgentState,
  AuditEvent,
  CatalogApp,
  CatalogAppsResponse,
  DiagnosticsApplyRequest,
  DiagnosticsApplyResponse,
  DiagnosticsComponentHealth,
  DiagnosticsServiceState,
  DiagnosticsSettings,
  DiagnosticsSettingsState,
  DiagnosticsSupportBundle,
  DiagnosticsValidateRequest,
  DisplayApplyRequest,
  DisplayApplyResponse,
  DisplayConfirmationLease,
  DisplayHeadConfiguration,
  DisplayHeadIdentity,
  DisplayLeaseActionRequest,
  DisplayLeaseState,
  DisplayMode,
  DisplayProtocolCapabilities,
  DisplayReconciliationState,
  DisplayReconciliationStatus,
  DisplaySettingsState,
  DisplayTopology,
  DisplayTransform,
  DisplayValidateRequest,
  CaptureClassification,
  EvidencePacket,
  LayoutActionKind,
  LayoutMode,
  LayoutState,
  LayoutSurface,
  LayoutWorkspace,
  LayoutZone,
  SettingsApplyOutcome,
  SettingsApplyOutcomeKind,
  SettingsAvailabilityState,
  SettingsCapability,
  SettingsCatalogEntry,
  SettingsCatalogResponse,
  SettingsCategory,
  SettingsDefaultsRequest,
  SettingsError,
  SettingsErrorCode,
  SettingsLoadRequest,
  SettingsModuleAvailability,
  SettingsModuleManifest,
  SettingsOperation,
  SettingsResetRequest,
  SettingsValidationIssue,
  SettingsValidationResponse,
  ShortcutApplyRequest,
  ShortcutApplyResponse,
  ShortcutAssignment,
  ShortcutDefinition,
  ShortcutKeymap,
  ShortcutSettingsState,
  ShortcutValidateRequest,
  SurfaceEvent,
  SurfaceEventKind,
  SurfaceGeometry,
  SurfaceLayoutParticipation,
  ThemeToken,
  VisualStatus,
  WindowLayoutMode,
  WindowLayoutRule,
  WindowManagementApplyRequest,
  WindowManagementApplyResponse,
  WindowManagementGaps,
  WindowManagementSettings,
  WindowManagementSettingsState,
  WindowManagementValidateRequest,
  WindowWorkspaceSummary,
} from './generated/contracts.js';

export type ClassifiedError =
  | { readonly kind: 'network'; readonly message: string; readonly status?: number }
  | { readonly kind: 'auth'; readonly message: string; readonly status: 401 | 403 }
  | { readonly kind: 'not-found'; readonly message: string; readonly status: 404 }
  | { readonly kind: 'server'; readonly message: string; readonly status: number }
  | { readonly kind: 'invalid-response'; readonly message: string }
  | { readonly kind: 'unknown'; readonly message: string; readonly status?: number };

export interface DeResultOk<T> {
  readonly ok: true;
  readonly value: T;
}

export interface DeResultError {
  readonly ok: false;
  readonly error: ClassifiedError;
}

export type DeResult<T> = DeResultOk<T> | DeResultError;
