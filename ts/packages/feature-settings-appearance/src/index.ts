...
export const appearanceSettingsPage: SettingsPageDefinition = {
  ...
  </style><section class="settings-module-panel" data-settings-module="appearance"><p id="appearance-status" role="status">Loading bundled themes…</p><div class="appearance-themes" id="appearance-themes"></div><p class="setting-detail">Preview affects this Settings window. Apply updates open shell surfaces and saves the theme for the next session.</p></section>`,
  renderClientController:()=>String.raw`(api)=>{
    const list=api.content.querySelector("#appearance-themes");const status=api.content.querySelector("#appearance-status");let previewing=false;let previewThemeId="";
    function authoritative(state){previewing=false;previewThemeId="";if(window.agoraTheme)window.agoraTheme.endPreview(state);}
    function preview(theme){previewing=true;previewThemeId=theme.id;if(window.agoraTheme)window.agoraTheme.preview(theme);status.textContent="Previewing "+theme.name+"; apply to keep it.";}
    function render(){const state=api.getModuleState(),draft=api.getDraft();if(!state||!draft)return;if(previewing&&!api.isDirty())authoritative(state);if(api.isDirty()&&draft.themeId!==previewThemeId){const selected=state.themes.find((theme)=>theme.id===draft.themeId);if(selected)preview(selected);}list.replaceChildren();state.themes.forEach((theme)=>{const label=document.createElement("label");label.className="appearance-theme";const row=document.createElement("span");const input=document.createElement("input");input.type="radio";input.name="appearance-theme";input.value=theme.id;input.checked=draft.themeId===theme.id;const name=document.createElement("strong");name.textContent=theme.name+(state.active.themeId===theme.id?" (active)":"");row.append(input,name);const swatch=document.createElement("span");swatch.className="appearance-swatch";swatch.setAttribute("aria-label",theme.name+" color preview");["--agora-bg","--agora-surface","--agora-accent"].forEach((token)=>{const cell=document.createElement("span");cell.style.background=theme.tokens[token]||"transparent";swatch.appendChild(cell);});input.addEventListener("change",()=>{api.getDraft().themeId=theme.id;preview(theme);api.markDirty();});label.append(row,swatch);list.appendChild(label);});if(!previewing)status.textContent="Choose a bundled theme to preview.";}
    return {render,applied(result){previewing=false;previewThemeId="";if(window.agoraTheme)window.agoraTheme.notifyApplied(result.state);},destroy(){authoritative(api.getModuleState());}};
  }`,
};
